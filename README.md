...
### 5.8 前端测试【新增·工作与面试高频】
- 5.8.1 为什么前端也需要测试
- 5.8.2 组件单元测试：Vitest + React Testing Library
- 5.8.3 Mock（网络请求、模块）
- 5.8.4 端到端测试基础：Playwright / Cypress
- 5.8.5 测试金字塔与覆盖率认知

### 5.9 无障碍访问 Accessibility (a11y)【新增·结合Shopify背景】
- 5.9.1 为什么无障碍访问重要（电商场景尤其相关）
- 5.9.2 语义化HTML与ARIA属性基础
- 5.9.3 键盘导航与焦点管理
- 5.9.4 自动化检测工具：axe / Lighthouse a11y审计
...
- 6.7.6 项目目录结构最佳实践
- 6.7.7 Tailwind CSS基础与技术选型【新增·工作高频】（对比CSS Modules / CSS-in-JS）
- 6.7.8 Core Web Vitals与Lighthouse【新增·工作高频】（LCP / INP / CLS，电商场景性能衡量）
...
- 9.3.1 http模块基础
- 9.3.2 处理请求与响应
- 9.3.3 手写一个最简单的服务器（不用框架，理解本质）

### 9.4 CommonJS模块系统
- 9.4.1 require / module.exports原理
- 9.4.2 模块加载机制

### 9.5 npm基础
- 9.5.1 package.json结构
- 9.5.2 依赖安装与管理
...
- 10.2.2 路由参数
- 10.2.3 用express.Router()拆分路由模块

### 10.3 中间件
- 10.3.1 中间件是什么（本质是函数管道）
- 10.3.2 执行顺序与next()
- 10.3.3 内置中间件express.json()解析请求体
...
## 第十一部分：GraphQL基础（结合Shopify背景重点学）【新增，提前】

### 11.1 为什么Shopify等电商平台转向GraphQL（REST逐步废弃的背景）
...
### 11.3 GraphQL vs REST：对比实战（对照第10部分学过的REST设计）

### 11.4 在Express中集成GraphQL（Apollo Server基础）
...
### 12.6 索引与查询性能【新增·面试高频】
- 12.6.1 索引原理（B-Tree基础认识）
- 12.6.2 什么场景该加索引
- 12.6.3 N+1查询问题及解决方案

### 12.7 事务（Transaction）【新增·工作高频】
- 12.7.1 ACID基本概念
- 12.7.2 什么场景必须用事务（如下单扣库存）

### 12.8 Prisma ORM
- 12.8.1 Schema定义
- 12.8.2 Migration工作流
- 12.8.3 Client查询API
- 12.8.4 Prisma中的事务API

### 12.9 🎯里程碑项目：把Todo List改造成真正连数据库
- 12.9.1 Schema设计
- 12.9.2 替换内存存储为数据库操作
- 12.9.3 联调测试
...
## 第十四部分：🎯项目1：博客系统
> 提前到认证学完后立刻练手，巩固CRUD+认证，避免拖到系统设计之后才第一次动手

### 14.1 需求与路由设计

### 14.2 分层架构实践：Controller / Service层拆分【结合第十五部分15.1提前用上】
...
### 15.1 分层架构（Controller / Service / Repository模式）【新增·工作高频】
- 15.1.1 为什么不能把所有逻辑堆在路由里
- 15.1.2 三层拆分实战
...
### 15.6 API安全加固【新增·工作高频】
- 15.6.1 helmet基础安全头
- 15.6.2 express-rate-limit限流中间件
- 15.6.3 CORS配置实战（结合4.6原理）

### 15.7 文件上传【新增·工作高频】
- 15.7.1 multer基础
- 15.7.2 上传到云存储（S3基础概念）

### 15.8 Webhook处理【新增·结合Shopify背景】
- 15.8.1 什么是Webhook
- 15.8.2 签名验证（防伪造请求）
- 15.8.3 幂等性处理（防重复消费）
...
- 15.9.1 分页 / 过滤 / 排序
- 15.9.2 API版本管理
- 15.9.3 OpenAPI / Swagger文档生成

### 15.10 NestJS
- 15.10.1 为什么用它（对比Express的痛点）
- 15.10.2 Controller / Service / Module
- 15.10.3 Dependency Injection（依赖注入）原理
- 15.10.4 装饰器（Decorator）基础概念

### 15.11 Redis基础
- 15.11.1 缓存场景实战（Cache-aside）
- 15.11.2 Session存储
...
### 16.1 Docker基础概念
- 16.1.1 Image / Container / Volume / Network
- 16.1.2 Dockerfile编写
...
### 16.5 CI/CD基础
- 16.5.1 GitHub Actions workflow语法
- 16.5.2 自动化测试与部署流水线

### 16.6 监控与错误追踪【新增·工作高频】
- 16.6.1 Sentry错误监控接入
- 16.6.2 健康检查接口（health check）基础认识
...
- 17.1.2 权限系统设计
- 17.1.3 文件上传系统设计
- 17.1.4 支付系统设计（Stripe集成思路）
- 17.1.5 消息通知系统设计
- 17.1.6 电商订单系统设计（结合Shopify背景重点练）
- 17.1.7 缓存设计实战
- 17.1.8 高并发基础概念（限流/降级/熔断）

---

## 第十八部分：项目实战（进阶项目）

### 18.1 项目2：迷你电商后台管理系统（核心简历项目）
- 18.1.1 需求与数据库设计
- 18.1.2 后端API开发（商品/订单/库存/权限）
- 18.1.3 前端管理界面开发
- 18.1.4 支付集成
- 18.1.5 测试与部署
- 18.1.6 架构文档撰写

### 18.2 项目3：SaaS系统（可选，加深多用户/订阅计费）

### 18.3 项目4：AI SaaS项目（可选，见第十九部分AI章节后再做）

### 18.4 项目复盘与文档整理

---

## 第十九部分：AI应用开发（可选，求职加分项）

- 19.1 LLM基础概念
- 19.2 Prompt Engineering基础
- 19.3 OpenAI API使用
- 19.4 Streaming响应处理
- 19.5 Function Calling
- 19.6 Embedding基础
- 19.7 Vector Database基础
- 19.8 RAG（检索增强生成）基础
- 19.9 MCP基础概念
- 19.10 Agent基础概念
- 19.11 AI SaaS开发实战（把前面全栈技能与AI结合）
...
### 20.1 分模块面试题库
- 20.1.1 JavaScript面试高频题
- 20.1.2 TypeScript面试高频题
- 20.1.3 浏览器面试高频题
- 20.1.4 HTTP面试高频题
- 20.1.5 React面试高频题
- 20.1.6 Next.js面试高频题
